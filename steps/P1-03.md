...
本セッションの 5 つのアプリのデプロイ方法は下記の方法を使います。
...
## 事前準備
1. 前のステップに開いた Git Bash ターミナル画面を引き続き使用します。
    ```bash
...
3. Visual Studio Code の横メニューの `Azure アイコン`をクリックします。SPRING APPS 下の `Sign in to Azure...` をクリックします。
...
## App の作成 
1. Spring APPS 欄に、`自分のアカウント`をクリックして展開します。前の手順に Azure CLI で作成した Spring APPS プランが表示されます。右クリックして `Create App` を選択します。
...
## App のデプロイ
1. customers-service を表示します。`Deploy` をクリックします。    
