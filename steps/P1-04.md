...
2. ハンズオン用の `Spring Apps` を選択します。次の画面に`アプリ`をクリックします。
    ...
    アプリの一覧が表示されます。プロビジョニングは全て`成功`していることを確認します。<br>
    <img src="../images/P1-04-view-success-status.png" width="900">

## 動作確認
1. アプリ一覧から`api-gateway` をクリックします。`URL` はアクセス用のエンドポイントですので、メモしておいてください。

    <img src="../images/P1-04-view-summary.png" width="900">

   アプリ一覧から`admin-server` をクリックします。`URL` をメモしておきます。  
...
4. 先ほどメモした `admin-server の URL` へアクセスします。
