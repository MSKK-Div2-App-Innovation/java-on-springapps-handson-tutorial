...
2. `監視対象`を選択します。次の画面に左側のメニューの `アプリケーション マップ` をクリックします。

    全てのコンポーネントおよび依存関係が表示されます。サービスのトポロジーを可視化できます。
...
## パフォーマンス
1. Application Insight ページの左側メニューの `パフォーマンス` をクリックします。
    ...
    データベース のアクセス件数および平均レスポンスタイムが表示されます。
...
## サービスのリアルタイム監視
1. Application Insight ページの左側メニューの `ライブ メトリック` をクリックします。

    Incoming Requests / Outgoing Requests および各サービスの失敗リクエスト数・CPU・メモリなどの情報がリアルタイムで表示されます。
