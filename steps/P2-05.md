...
ソースコードを修正してリポジトリにコミットすると、ワークフローが自動的に起動され、速やかに修正内容を Container App にデプロイするよう、 GitHub Actions を設定します。
...
2.  API アプリの リポジトリの `springapps-petclinic-microservices/spring-petclinic-api-gateway/src/main/resources/static/scripts/fragments/welcome.html` にメッセージを追加し、`main` ブランチにコミットします。 
