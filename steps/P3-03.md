...
GitHub Actions のワークフローは一時保存したアプリの jar ファイルをダウンロードして、前の手順で確認した非稼働環境へ jar ファイルをデプロイします。
デプロイ完了後、疎通確認するために、非稼働環境のエンドポイントを取得します。最後に非稼働環境にアクセスして、正常に稼働しているかどうかテストを実施します。
...
2. `Actions`タブにて、実行済みのワークフローを選択します。次は、`deploy` ジョブをクリックします。
    ...
    ワークフロー処理がエラーなく完了し、非稼働環境エンドポイントの疎通テストが成功した ( HTTP/2 200 が出力される ) ことを確認します。
